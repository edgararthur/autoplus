...
const Login = () => {
  ...
  const [userType, setUserType] = useState('dealer'); // Default to dealer for dealer platform
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Navigate to the dealer dashboard when login is successful
        navigate('/dashboard');
      } else {
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div>
          ...
          <select
            ...
            value={userType}
            disabled
            className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-neutral-300 bg-neutral-100 cursor-not-allowed focus:outline-none sm:text-sm rounded-md"
          >
            <option value="dealer">Dealer</option>
          </select>
          <p className="mt-1 text-xs text-neutral-500">This platform is for vehicle dealers only</p>
        </div>
